...
def create_vault():
    plugin = os.path.join(".test", ".obsidian", "plugins")
    ...
    with zipfile.ZipFile(".vscode/scripts/vault.zip", "r") as z:
        z.extractall(os.path.join(".test", ".obsidian"))

def copy_plugin():
    if os.path.exists(".test/.obsidian/plugins/image-inline"):
        shutil.rmtree(".test/.obsidian/plugins/image-inline")
    shutil.copytree("output", os.path.join(".test", ".obsidian", "plugins", "image-inline"))
...
def open_vault():
    with temp_register(".test"):
        open_obsidian_vault(".test")

if __name__ == "__main__":
    if os.path.exists("output/manifest.json"):
        os.remove("output/manifest.json")
    shutil.copyfile("manifest.json", "output/manifest.json")
