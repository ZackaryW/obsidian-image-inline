...
export class ConvertAllModal extends Modal {
    private onConfirm: (filters: { skipInternalLinks: boolean; skipRemoteLinks: boolean, customFilters: string[] }) => void;
    private skipInternalLinks: boolean = false;
    private skipRemoteLinks: boolean = false;
    private customFilters: string[] = [];

    constructor(app: App, onConfirm: (filters: { skipInternalLinks: boolean; skipRemoteLinks: boolean, customFilters: string[] }) => void) {
        super(app);
    ...
    onOpen() {
        ...
        contentEl.createEl('p', { text: 'Do you want to convert all images to base64? You can also add filter schemes to skip certain images.' });

        new Setting(contentEl)
            .setName('Skip links starting with ![[xxx]]')
            .addToggle(toggle => toggle
                .setValue(this.skipInternalLinks)
                .onChange(value => {
        ...
        new Setting(contentEl)
            ...
            .setDesc('Comma-separated list of keywords to skip (e.g., http, data)')
            .addText(text => text
...
export function shouldSkipImage(link: string, imagePath: string, filters: { skipInternalLinks: boolean; skipRemoteLinks: boolean; customFilters: string[] }): boolean {
